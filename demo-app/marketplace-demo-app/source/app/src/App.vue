<template>
  <div id="app" class="antialiased leading-tight">
    <div
      class="header-wrap fixed w-screen pt-2 pb-4 pl-8 pr-8 bg-white shadow-xl"
    >
    ...
    <div class="content-wrapper container mx-auto max-w-6xl">
      <div class="errors">
        ...
        <error v-if="assetUploadError">
          ...
          <template v-slot:body>
            <code>
              <pre class="overflow-x-auto pt-4 pb-4">{{
                assetUploadError
              }}</pre>
            </code>
          </template>
      ...
      <div v-if="dataIsLoaded">
        ...
        <div
          ...
          class="results-info mb-4 mt-4 text-gray-600"
        >
          <p>
            Viewing {{ items.length }} results
            <span v-if="searchQuery">
              for
              <strong class="bg-gray-200 rounded px-2 py-1">{{
                searchQuery
              }}</strong>
            </span>
          </p>
      ...
      <div
        v-else
        class="loading-screen h-screen flex items-center justify-center text-pink"
      >
...
export default {
  ...
  metaInfo: {
    title: "Home",
    titleTemplate: "%s | K9O Marketplace",
    htmlAttrs: {
      lang: "en",
    },
  },
